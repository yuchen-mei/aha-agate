...
### Formal Verification
This checkout does not include a formal backend for `SymbolicTester`. The
assume/guarantee interface remains available for constrained random simulation
through the Verilator target.
...
(verilator) and observe the readability of the output (is it easy to understand
...
### Formal
The main issue with the `assume`/`guarantee` interface is the translation of the
specification language to a target language expected by a formal backend. To
improve the state of this, the specification language should be precisely
defined, and the translation should be done using a generic AST rewriter and
serializer.  This specification language should also include facilities for
describing temporal properties (currently we only support "combinational"
properties).
...
to setup the state of the circuit under test. Then, this state can be dumped and
provided to a formal backend as the state.  In some cases, this approach will be
less expensive than having the solver setup the state using a mechanism like the
`ets` which requires unrolling in time.
