...
public:
    ...
    void visit(SequentialStmtBlock* block) override {
        ...
        // If the parent generator has the disable switch, don't touch the block...
        if (generator->clk_en_no_touch()) return;
...
public:
    ...
    void visit(Generator* generator) override {
        if (generator->external() || generator->is_stub() || generator->sync_reset_no_touch()) return;
        if (!run_pass_) return;
        Port* port;
